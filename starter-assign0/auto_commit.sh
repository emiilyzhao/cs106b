...
commit_and_push() {
  ...
  if git commit -m "Auto-commit #$count from $commit_source at $ts"; then
    :
  else
  ...
  if git remote get-url origin >/dev/null 2>&1; then
    git push -u origin HEAD || echo "[auto_commit] Push failed (continuing)."
  else
...
# MODE == --run
if [[ "$FORCE_RUN" -ne 1 ]] && should_skip_run "${WATCH[@]}"; then
