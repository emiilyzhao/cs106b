...
function Ensure-Repo {
  if (-not (Test-Path ".git")) {
    ...
    git config user.name  "Student Name"          2>&1 | Out-Null
    git config user.email "student@example.com"   2>&1 | Out-Null
...
function Commit-And-Push([int]$Count, [string]$OutputFile, [string[]]$AlsoAdd) {
  ...
  git commit --no-verify -m ("Auto-commit #{0} from {1} at {2}" -f $Count, $commitSource, $ts)
