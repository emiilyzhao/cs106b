...
bool Puzzle::canMatchEdge(Tile tile, GridLocation loc, Direction dir) const {
    ...
    if (dir == NORTH) {
        if (loc.row == 0)
            return true;
        neighbor = GridLocation(loc.row-1, loc.col);
        neighborEdge = tileAt(neighbor).getEdge(SOUTH);
    }
    // east
    if (dir == EAST) {
        if (loc.col == _grid.numCols()-1)
            return true;
        neighbor = GridLocation(loc.row, loc.col+1);
        neighborEdge = tileAt(neighbor).getEdge(WEST);
    }
    // south
    if (dir == SOUTH) {
        if (loc.row == _grid.numRows()-1)
            return true;
        neighbor = GridLocation(loc.row+1, loc.col);
        neighborEdge = tileAt(neighbor).getEdge(NORTH);
    }
    // west
    if (dir == WEST) {
        if (loc.col == 0)
            return true;
        neighbor = GridLocation(loc.row, loc.col-1);
        neighborEdge = tileAt(neighbor).getEdge(EAST);
    }
    ...
    // complement
    if (isComplement(tile.getEdge(dir), neighborEdge))
        return true;
...
// student tests for canMatchEdge

STUDENT_TEST("canMatchEdge returns true for blank neighborhoods") {
    Map <string, string> pairs = {{"A","a"}, {"a", "A"}, {"B","b"}, {"b", "B"}, {"C","c"}, {"c","C"}};
    Puzzle puzzle;
    puzzle.configure(2, 2, pairs);

    Tile tile1("A", "B", "C", "a");
    Tile tile2("B", "C", "A", "b");
    puzzle.add(tile1);

    EXPECT(puzzle.canMatchEdge(tile2, {0, 1}, SOUTH));
    EXPECT(puzzle.canMatchEdge(tile2, {1,1}, NORTH));

}

STUDENT_TEST("canMatchEdge returns true when off grid") {
    Map <string, string> pairs = {{"A","a"}, {"a", "A"}, {"B","b"}, {"b", "B"}, {"C","c"}, {"c","C"}};
    Puzzle puzzle;
    puzzle.configure(2, 2, pairs);
    ...
    EXPECT(puzzle.canMatchEdge(tile, {0,0}, NORTH));
    EXPECT(puzzle.canMatchEdge(tile, {0,0}, WEST));
    EXPECT(puzzle.canMatchEdge(tile, {0,1}, NORTH));
    EXPECT(puzzle.canMatchEdge(tile, {0,1}, EAST));
    EXPECT(puzzle.canMatchEdge(tile, {1,0}, SOUTH));
    EXPECT(puzzle.canMatchEdge(tile, {1,0}, WEST));
}

STUDENT_TEST("canMatchEdge returns false when not matching") {
    Map <string, string> pairs = {{"A","a"}, {"a", "A"}, {"B","b"}, {"b", "B"}, {"C","c"}, {"c","C"}};
    Puzzle puzzle;
    puzzle.configure(2, 2, pairs);
    ...
    EXPECT(!puzzle.canMatchEdge(tile2, {1, 0}, NORTH));
