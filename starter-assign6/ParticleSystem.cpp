...
/* This function is the constructor for the particle system.
 * The constructor creates an empty list of particles, so the head
 * and tail are null and the count of particles is initialized to 0.
 */
...
/* This function is the destructor of the particle system.
 * this iterates through the list, holding the location of
 * each particle while moving the pointer to the next to
 * delete all particles.
 * the head and tail and count are reinitialized to null and 0.
 */
ParticleSystem::~ParticleSystem() {
    ParticleCell* current = _head;
    while (current != nullptr) {
        ParticleCell* holder = current->next;
        delete current;
        current = holder;
    }
    _head = nullptr;
...
/* this function returns the count of the number of particles
...
/* This function returns whether the particle is valid. this is done by
 * checking whether it is within bounds of the scene and whether the lifetime
 * has reached 0. if not valid, the function returns true
 */
bool notValid(Particle part) {
    bool onScreen = part.x < 0 || part.x >= SCENE_WIDTH || part.y < 0 || part.y >= SCENE_HEIGHT;
    bool lifetime = part.lifetime < 0;
    return (onScreen || lifetime);
}

/* this function adds a particle to the particle system, only if it is valid.
 */
void ParticleSystem::add(const Particle& data) {
    // dont add if not valid
    if (notValid(data))
        return;

    // create new particle with correct next/prev designations
    ParticleCell* newCell = new ParticleCell;
    newCell->particle = data;
    newCell->next = nullptr;
    newCell->prev = _tail;

    // if list starts off empty
    if (_head == nullptr) {
        _head = newCell;
        _tail = newCell;
    }
    // the list already has particles
    else {
        _tail->next = newCell; // fix next for last particle
        _tail = newCell; // reinitialize tail
    }
...
/* this function iterates through all particles in the list and draws them.
 */
void ParticleSystem::drawParticles() const {

    for (ParticleCell* cur = _head; cur != nullptr; cur = cur->next) {
        Particle particle = cur->particle;
        drawParticle(particle.x, particle.y, particle.color);
    }
}

/* this function iterates through all particles and moves them. different
 * particles types are moved differently. if the particles are no longer
 * valid after they are moved, they are removed.
 */
void ParticleSystem::moveParticles() {
    ...
    while (cur != nullptr) {
        ...
        // special particle: ballistic
        if (cur->particle.type == ParticleType::BALLISTIC)
            cur->particle.dy++;

        // special particle: firework
        if (cur->particle.type == ParticleType::FIREWORK) {
            cur->particle.dy++;
            // explodes
            if (cur->particle.lifetime <= 0) {
                ...
                for (int i = 0; i < 50; i++) {
                    ...
                    p.x = cur->particle.x;
                    p.y = cur->particle.y;
                    p.dx = randomInteger(-3, 3);
                    p.dy = randomInteger(-3, 3);
                    p.lifetime = randomInteger(2, 10);
                    p.color = color;
                    add(p);
                }
            }
        ...
        // check if still valid; if not, remove
        if (notValid(cur->particle)) {
...
STUDENT_TEST("Milestone 1: initial constructor private variables are correct") {
    ...
    EXPECT_EQUAL(system._count, 0);
}

STUDENT_TEST("Milestone 1: private variables correct when one particle added") {
    ParticleSystem system;
    Particle particle;
    ...
    // check new variables are correct
    EXPECT_EQUAL(system.numParticles(), 1);
    EXPECT_NOT_EQUAL(system._head, nullptr);
    EXPECT_NOT_EQUAL(system._tail, nullptr);
    EXPECT_EQUAL(system._count, 1);

    // check pointers are right
    EXPECT_EQUAL(system._head->prev, nullptr);
    EXPECT_EQUAL(system._head->next, nullptr);
}

STUDENT_TEST("Milestone 1: private variables correct when two particles added") {
    ParticleSystem system;
    Particle particle1;
    Particle particle2;

    // add particle to system, get first details
    system.add(particle1);
    auto* head1 = system._head;
    auto* tail1 = system._tail;

    // add second particle
    system.add(particle2);

    // check new variables are correct as expected
    EXPECT_EQUAL(system.numParticles(), 2);
    EXPECT_EQUAL(system._count, 2);
    EXPECT_EQUAL(system._head, head1);
    EXPECT_NOT_EQUAL(system._tail, tail1);

    // check two nodes connected
    EXPECT_EQUAL(system._head->next, system._tail);
    EXPECT_EQUAL(system._tail->prev, system._head);

    // check end pointers are right
    EXPECT_EQUAL(system._head->prev, nullptr);
    EXPECT_EQUAL(system._tail->next, nullptr);
}

STUDENT_TEST("Milestone 5: test that firework type is correct") {
    ParticleSystem system;
    Particle particle;
    particle.type = ParticleType::FIREWORK;
    particle.x = SCENE_WIDTH/3;
    particle.y = SCENE_HEIGHT/3;
    particle.dx = 0;
    particle.dy = 0;
    particle.lifetime = 0;
